...
The output notebook is designed to run on a MacBook (Apple Silicon M2 Max
or similar). It is split into 10 cells so each conversion / eval step can
be debugged independently — long-running cells do not depend on previous
cell output beyond the imports established at the top.
"""
...
CELLS.append(md(
    ...
    "\n"
    "**Prerequisite:** Run `python scripts/edge/setup_macbook_mirror.py ...` "
    "FIRST to extract DF40 ZIPs locally + stage checkpoints + rewrite CSV paths. "
    "See `docs/EDGE_EVAL_MACBOOK_GUIDE.md` for the exact commands.\n"
    "\n"
    "**Caveat:** M2 Max is a development-class chip, NOT a representative "
    "edge device. Latency on M2 Max will be 4-8x faster than on a "
    "Snapdragon 7-series CPU. Report numbers with this caveat in the thesis.\n"
))

# --- Cell 1: Sanity check environment ------------------------------------ #
CELLS.append(md("## 1. Environment + Hardware Check"))
CELLS.append(code(
    "import platform\n"
    "import sys\n"
    "\n"
    ...
    f"        f'Got {{platform.machine()!r}}. CoreML benchmarks will not run.'\n"
    ")\n"
    "assert platform.system() == 'Darwin', (\n"
    f"    f'macOS expected, got {{platform.system()!r}}.'\n"
    ")\n"
    "\n"
...
CELLS.append(md(
    ...
    "\n"
    "Use `uv` if available (fast), otherwise plain pip. **This cell can be "
    "skipped if you've already installed `requirements-macbook.txt`.**\n"
))
CELLS.append(code(
    "import shutil\n"
    "import subprocess\n"
...
# --- Cell 4: Pilot conversion + sanity check ----------------------------- #
CELLS.append(md(
    "## 4. Pilot Conversion + Sanity Check\n"
    "\n"
    "Convert ONE checkpoint to all 4 formats (TFLite fp32, TFLite int8, "
    "CoreML fp32, CoreML int8) and verify numerical match against the "
    "PyTorch reference. **Stop here if any sanity check fails** — debug "
...
# --- Cell 5: Mass conversion (orchestrator) ------------------------------ #
CELLS.append(md(
    "## 5. Mass Conversion (9 checkpoints x up to 4 formats)\n"
    "\n"
    "Runs `scripts/edge/run_edge_eval_macbook.py` end-to-end. Long cell "
    "(~30-60 min depending on chip). Output: converted artifacts + full "
    "AUC + latency JSON + Markdown summary.\n"
    "\n"
    "To debug individual steps, prefer the `--pilot-only` flag from CLI; "
    "the full run is here for one-click execution.\n"
))
CELLS.append(code(
...
# --- Cell 7: Quick figure (optional) -------------------------------------- #
CELLS.append(md(
    "## 7. Optional: Quick Figure (Latency vs Size)\n"
    "\n"
    "Generates a quick scatter plot of model size vs latency, color-coded "
    "by runtime/mode. Saves to `edge_results/results/latency_vs_size.png`.\n"
))
CELLS.append(code(
...
# --- Cell 8: Open Markdown summary ---------------------------------------- #
CELLS.append(md(
    "## 8. Open Final Summary\n"
    "\n"
    "Open the Markdown summary file — copy/paste relevant tables into the "
    "thesis or use the figures generated above.\n"
))
CELLS.append(code(
