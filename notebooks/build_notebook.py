...
CELLS.append(md(
    "## 4. Download DF40 to Drive (one-time, ~55 GB)\n"
    "\n"
    "Three pieces, all from the official [DF40 release](https://github.com/YZY-stack/DF40):\n"
    "\n"
    "| Piece | Size | ID | Target on Drive |\n"
    "|---|---|---|---|\n"
    "| `DF40_train` (fakes, ff-only) | ~50 GB | folder `1U8meBbqVvmUkc5GD0jxct6xe6Gwk9wKD` | `datasets/raw/df40/` |\n"
    "| FF++ real crops (ZIP) | ~3 GB | file `1dHJdS0NZ6wpewbGA5B0PdIBS9gz28pdb` | `datasets/raw/df40_real/ff_real/` |\n"
    "| Celeb-DF real crops (ZIP) | ~2 GB | file `1FGZ3aYsF-Yru50rPLoT5ef8-2Nkt4uBw` | `datasets/raw/df40_real/cdf_real/` |\n"
    "\n"
    "Nine \"unknown\" methods (MidJourney, whichfaceisreal, stargan, starganv2, styleclip, e4e, CollabDiff) "
    "ship their own real frames inside the method folder (`<tech>/real/`), so no extra download for those.\n"
    "\n"
    "The cell is idempotent — it skips pieces that are already present.\n"
))
CELLS.append(code(
    ...
    "\n"
    "raw_root       = Path('/content/drive/MyDrive/CKD_Thesis/datasets/raw')\n"
    "df40_root      = raw_root / 'df40'\n"
    "df40_real_root = raw_root / 'df40_real'\n"
    "ff_real_dir    = df40_real_root / 'ff_real'\n"
    "cdf_real_dir   = df40_real_root / 'cdf_real'\n"
    "for d in (df40_root, ff_real_dir, cdf_real_dir):\n"
    "    d.mkdir(parents=True, exist_ok=True)\n"
    "\n"
    "# 4a) DF40 fakes (folder).\n"
    "if not any(df40_root.iterdir()):\n"
    "    print('Downloading DF40_train (~50 GB) — this will take a while…')\n"
    "    !gdown --folder https://drive.google.com/drive/folders/$DF40_TRAIN_FOLDER_ID \\\n"
    "        -O $df40_root --remaining-ok\n"
    "else:\n"
    "    print('DF40 fake root already populated — skipping.')\n"
    "\n"
    "def _gdown_and_unzip(file_id: str, tmp_zip: Path, extract_to: Path) -> None:\n"
    "    if any(extract_to.iterdir()):\n"
    "        print(f'{extract_to.name} already populated — skipping.')\n"
    "        return\n"
    "    print(f'Downloading {tmp_zip.name} …')\n"
    "    subprocess.run(['gdown', '--id', file_id, '-O', str(tmp_zip)], check=True)\n"
    ...
    "_gdown_and_unzip(\n"
    "    FF_REAL_FILE_ID,\n"
    "    Path('/content/ff_real.zip'),\n"
    "    ff_real_dir,\n"
    ")\n"
    "# 4c) Celeb-DF real ZIP.\n"
    "_gdown_and_unzip(\n"
    "    CDF_REAL_FILE_ID,\n"
    "    Path('/content/cdf_real.zip'),\n"
    "    cdf_real_dir,\n"
    ")\n"
    "\n"
    "print('\\nDF40 fake techniques present:')\n"
    "!ls $df40_root | head -60\n"
    "print('\\nDF40 real tree (top 2 levels):')\n"
    "!ls $df40_real_root\n"
    "!ls $df40_real_root/ff_real 2>/dev/null | head -5\n"
    "!ls $df40_real_root/cdf_real 2>/dev/null | head -5\n"
))
